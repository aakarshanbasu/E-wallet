...
Class.forName("oracle.jdbc.OracleDriver");
            ...
            ResultSet res1=stm.executeQuery("select * from dth_provider where name='"+boardup+"'");
            if(res1.next())
            {
                int id=res1.getInt("id");
                ResultSet res2=stm.executeQuery("select * from dth_bill where acc_no='"+acc+"'");
                if(res2.next())
                {
//                if(acc.equals(acc_confirm))
//                {
                    ResultSet res3=stm.executeQuery("select * from dth_bill where acc_no='"+acc+"' and id='"+id+"'");
                    if(res3.next())
                    {
...
<div>
    <center>
        ...
        <form action="pay_DTH_bill.jsp" method="post">
...
%>
            ...
            else
            {
                %>
                <script type="text/javascript">
                                    alert('Incorrect DTH Provider Name');
                                    window.location = "home.jsp";
                                </script>
