...
Class.forName("oracle.jdbc.OracleDriver");
            ...
            ResultSet res1=stm.executeQuery("select * from electricity_board where name='"+boardup+"'");
            if(res1.next())
            {
                int id=res1.getInt("id");
                ResultSet res2=stm.executeQuery("select * from electricity_bill where acc_no='"+acc+"'");
                if(res2.next())
                {
//                if(acc.equals(acc_confirm))
//                {
                    ResultSet res3=stm.executeQuery("select * from electricity_bill where acc_no='"+acc+"' and id='"+id+"'");
                    if(res3.next())
                    {
...
<div>
    <center>
        ...
        <form action="pay_electricity_bill.jsp" method="post">
...
%>
            ...
            else
            {
                %>
                <script type="text/javascript">
                                    alert('Incorrect Electricity Board Name');
                                    window.location = "home.jsp";
                                </script>
